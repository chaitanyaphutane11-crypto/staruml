...
function parsePlantUML(text) {
  ...
  // Parse actors (sequence diagram participants)
...
function createAssociationWithRefs(assoc, classes) {
  const sourceClass = classes.find(c => c.name === assoc.sourceName);
  const targetClass = classes.find(c => c.name === assoc.targetName);
  if (!sourceClass || !targetClass) {
    console.warn(`Skipping unresolved association: ${assoc.sourceName} --> ${assoc.targetName}`);
    return null;
  }
  return {
    _type: "UMLAssociation",
...
const assocModels = associations
  .map(a => createAssociationWithRefs(a, classes))
  .filter(Boolean);
...
const noteViews = notes.map((note) => {
  const targetClass = classes.find(c => c.name === note.targetName);
